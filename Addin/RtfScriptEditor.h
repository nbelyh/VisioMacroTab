...
public:
   ...
   {
      WCHAR szName[16];
      KEYWORD(LPCWSTR pstrText)
      {
         ATLASSERT(::lstrlen(pstrText)<sizeof(szName)/sizeof(WCHAR));
         ::lstrcpy(szName, pstrText);
      }
      bool operator==(LPCWSTR pstrText) const
      {
         return ::lstrcmp(szName, pstrText) == 0;
      }
   };
   ...
   CSimpleArray<KEYWORD> m_aGlobals;
   CSimpleArray<KEYWORD> m_aConstants;
   WCHAR m_szCommentSL[6];
   WCHAR m_szCommentMLs[6];
   WCHAR m_szCommentMLe[6];
   WCHAR m_chQuote;
   ...
   BOOL AddKeyword(LPCWSTR pstrText)
   {
   ...
   void ClearGlobals()
   {
	   m_aGlobals.RemoveAll();
   }
   BOOL AddGlobal(LPCWSTR pstrText)
   {
	   ATLASSERT(!::IsBadStringPtr(pstrText,-1));
	   KEYWORD k = pstrText;
	   return m_aGlobals.Add(k);
   }
   ...
   BOOL AddConstant(LPCWSTR pstrText)
   {
      ATLASSERT(!::IsBadStringPtr(pstrText,-1));
      KEYWORD k = pstrText;
      return m_aConstants.Add(k);
   }
   BOOL SetSyntax(int iType, LPCWSTR pstrText)
   {
      switch( iType ) {
      case SC_COMMENT:
         ::lstrcpyn(m_szCommentSL, pstrText, 5);
         return TRUE;
      case SC_STRING:
   ...
   void _Init()
   {
      ...
      ::lstrcpy(cfDefault.szFaceName, L"Courier New");
      ...
      ::lstrcpy(m_szCommentSL, L"//");
      ::lstrcpy(m_szCommentMLs, L"/*");
	  ::lstrcpy(m_szCommentMLe, L"*/");
      m_chQuote = '\"';
   ...
   int _FindKeyword(LPCWSTR pstrText) const
   {
	   ATLASSERT(!::IsBadStringPtr(pstrText,-1));
	   for( int i = 0; i < m_aKeywords.GetSize(); i++ ) {
		   if( m_bCaseSensitive ) {
			   ...
			   if( ::lstrcmpi(m_aKeywords[i].szName, pstrText) == 0 ) return i;
		   }
	   }
	   return -1;
   }
   int _FindGlobal(LPCWSTR pstrText) const
   {
      ATLASSERT(!::IsBadStringPtr(pstrText,-1));
      for( int i = 0; i < m_aGlobals.GetSize(); i++ ) {
         if( m_bCaseSensitive ) {
            if( m_aGlobals[i] == pstrText ) return i;
         }
         else {
            if( ::lstrcmpi(m_aGlobals[i].szName, pstrText) == 0 ) return i;
         }
      }
      return -1;
   }
   int _FindConstant(LPCWSTR pstrText) const
   {
      ATLASSERT(!::IsBadStringPtr(pstrText,-1));
      for( int i = 0; i < m_aConstants.GetSize(); i++ ) {
         if( m_bCaseSensitive ) {
            ...
            if( ::lstrcmpi(m_aConstants[i].szName, pstrText) == 0 ) return i;
         }
      }
      return -1;
   }
   inline bool _IsStringQuote(WCHAR ch) const
   {
      return ch == m_chQuote;
   }
   ...
   void _ChangeCase(int nStart, int nEnd, LPCWSTR pstrText)
   {
   ...
   void _FormatTextRange(int nStart, int nEnd)
   {
      ...
      LPWSTR pBuffer = NULL;
      ATLTRY(pBuffer = (LPWSTR) malloc((nEnd - nStart + 1) * sizeof(WCHAR)));
      if( pBuffer == NULL ) return; // Urgh, out of memory!
      long nLen = GetSelText(pBuffer);
      ATLASSERT(nLen<=nEnd-nStart);
      pBuffer[nLen] = '\0';

      LPCWSTR pStart = pBuffer;
      LPWSTR pPtr = pBuffer;
      LPWSTR pSymbolStart = NULL;
      SYNTAXCOLOR ic = { 0 };
      ...
      while( *pPtr ) {
         ...
         { 
            ...
            if( ch == chQuote ) pPtr = ::CharNext(pPtr);
         ...
         { 
            // Process numbers
            pSymbolStart = pPtr;
            while( *pPtr >= '0' && *pPtr <= '9' ) pPtr = ::CharNext(pPtr);
         ...
         { 
            ...
            *pPtr = '\0';
            ...
            else 
            {
               ...
               {
			      nPos = _FindGlobal(pSymbolStart);
				  if( nPos >= 0 ) {
					  _ChangeCase(nStart + pSymbolStart - pBuffer, nStart + pPtr - pBuffer, 
						  m_aGlobals[nPos].szName);
					  ic = m_aColors[SC_GLOBAL];
				  }
				  else {
					  pSymbolStart = NULL;
				  }
